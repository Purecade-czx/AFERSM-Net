...
class FCN(nn.Module):
    ...
    def __init__(self, c_in, layers=[128, 256, 128], kss=[7, 5, 3], ksse=[5, 3, 1], clf=True, reduction=16):
        ...
        self.conv1 = nn.Conv1d(c_in, layers[0], kernel_size=kss[0], stride=1, padding='same')
        self.conv2 = nn.Conv1d(c_in, layers[0], kernel_size=ksse[0], stride=1, padding='same')
        self.conv3 = nn.Conv1d(layers[1], layers[1], kernel_size=kss[1], stride=1, padding='same')
        self.conv4 = nn.Conv1d(layers[1], layers[2], kernel_size=kss[2], stride=1, padding='same')
        self.bnconv = nn.BatchNorm1d(layers[0]*2)
        self.bnconv1 = nn.BatchNorm1d(layers[1]*2)
        ...
        self.relu = nn.ReLU()
        self.se = SELayer(layers[1], reduction)
        self.convblock2 = ConvBlock(layers[0], layers[1], ks=kss[1])
        self.se1 = SELayer(layers[0], reduction)
    ...
    def forward(self, x):
        ...
        # y1 = self.conv1(x)
        # y2 = self.conv2(x)
        # y_con = torch.cat([y1,y2],dim=1)
        # y_con = self.bnconv(y_con)
        # y_con = self.relu(y_con)
        # # y_con = self.max_pool1(y_con)
        # # y_con = self.gap(y_con)
        # y_con = self.se(y_con)
        # y3 = self.conv3(y_con)
        # # y4 = self.convblock4(y2)
        # y_con1 = self.bn2(y3)
        # y_con1 = self.relu(y_con1)
        # # y_con1 = self.max_pool2(y_con1)
        # # y_con1 = self.gap(y_con1)
        # y_con1 = self.se(y_con1)
        #
        # return y_con1
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers,  inchannel=52, activity_num=6, location_num=16):
        ...
        self.lstm = LSTM(256, 20, 2, activity_num)
        self.droup = nn.Dropout(p=0.2)
        ...
        self.loc_fc_f = nn.Linear(512, location_num)
    ...
    def forward(self, x):
        ...
        # print('lstm_x is:',lstm_x.shape)
        # lstm_x = self.lstm(lstm_x)
        # print('the lstm_x is:',lstm_x.shape)
        # lstm_x = torch.tensor(lstm_x)
        # lstm_x = lstm_x.view(lstm_x.size(0), -1)
